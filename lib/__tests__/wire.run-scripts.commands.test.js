...
describe('wire: run-scripts: commands', () => {
  ...
  it.each([
    ...
    {
      exitCode: 0,
      commands: { release: ['semantic-release'] },
      processArgs: ['release', '--dry-run', '--branches', 'jest'],
      expectedRunArgs: [
        [
          {
            args: ['--dry-run', '--branches', 'jest'],
            name: 'semantic-release',
            options: {
              nodeArgs: [],
            },
            path: 'path/to/semantic-release.js',
          },
        ],
      ],
    },
