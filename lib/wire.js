...
function wire(scriptPaths, options = {}) {
  ...
  async function run() {
    ...
    try {
      ...
      let scriptIndex = args.findIndex(isScript)
      let commandIndex = args.findIndex(isCustomCommand)
      let index = null

      // If both a script command and custom command are found,
      // choose the one that's found earlier in the array,
      // and negate the other one.
      if (scriptIndex !== -1 && commandIndex !== -1) {
        if (scriptIndex < commandIndex) {
          commandIndex = -1
        } else {
          scriptIndex = -1
        }
